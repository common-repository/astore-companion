<?php
if(!class_exists('AStorePosts_Widget')){
 class AStorePosts_Widget extends WP_Widget {
    
	public function __construct() {
		
		parent::__construct(
			'astore_recent_posts', // Base ID
			__( 'AStore: Recent Posts', 'astore-companion' ), // Name
			array( 'description' => __( 'Recent Posts.', 'astore-companion' ), ) // Args
		);
		
	}
 	function form( $instance ) {
 	    $defaults = array('list_num' => '4','style' => '1', 'title' => __( 'Recent Posts', 'astore-companion' ));
...
<p>
  <label for="<?php echo $this->get_field_id( 'list_num' ); ?>">
    <?php _e('Recent Posts List Num', 'astore-companion'); ?>
    :</label>
  <br />
...
<?php
	...
	function widget( $args, $instance ) {
 		extract( $args );
 	    $title    = apply_filters(__('Recent Posts', 'astore-companion'), esc_attr($instance['title']) );
...
function astore_companion_recent_posts(){
	
	register_widget('AStorePosts_Widget');
						
}
add_action( 'widgets_init', 'astore_companion_recent_posts' );
